...
$pdf->SetTitle('Laporan Daftar Pemilih Tetap (DPT)');
...
foreach ($voters_by_const as $kode => $list) {
    ...
    // Judul & Barcode
    $pdf->SetFont('helvetica', 'B', 14);
    $pdf->Cell(0, 8, 'DAFTAR PEMILIH TETAP', 0, 1, 'C');
    $pdf->SetFont('helvetica', 'B', 12);
    $pdf->Cell(0, 6, $election_data['nama_pemilihan'] ?? 'PEMILIHAN OSIS', 0, 1, 'C');
    $pdf->SetFont('helvetica', 'B', 12);
    ...
    $pdf->write1DBarcode($barcode_val, 'C39', 200, $pdf->GetY() - 19, '', 12, 0.4, $style, 'N');
    $pdf->Ln(8);
